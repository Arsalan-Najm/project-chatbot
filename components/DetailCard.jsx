...
const DetailCard = ({ data }) => {
  return (
    <div>
      <div>
        <div className='w-full grid lg:grid-cols-3 2xl:grid-cols-4 gap-6'>
          {data?.map((card) => (
            <div
              className='relative flex flex-col w-full justify-between py-8 px-6 rounded-lg overflow-hidden bg-[#1B1E28] shadow-md'
              style={{
                  ...
                  <ul className='w-full mt-2 pt-2 border-t-[1px] border-sub-200'>
                    {card?.games?.slice(0, 3)?.map((games) => (
                      <li className='flex justify-between w-full mb-[6px] font-normal text-sm'>
                        <Link href={'/platforms'} className='underline overflow-hidden whitespace-nowrap text-sub-100'>
                          {games?.name}
                        </Link>
